...
> **Note (migration):** In codex this agent used MCP server(s) `chrome_devtools` (http://localhost:3000/mcp). Configure the equivalent MCP server in `.claude/settings.json` (or via `claude mcp add`) for full parity.
