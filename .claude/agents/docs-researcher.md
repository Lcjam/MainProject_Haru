...
> **Note (migration):** In codex this agent used MCP server(s) `openaiDeveloperDocs` (https://developers.openai.com/mcp). Configure the equivalent MCP server in `.claude/settings.json` (or via `claude mcp add`) for full parity.
